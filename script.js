document.addEventListener('DOMContentLoaded', () => {
  ...
  if(bg) {
    ...
    const totalParticles = 40; // fewer for clarity
  ...
  function updateDashboardProgress() {
    ...
    const fillDuration = 12*60*60*1000;
    ...
    if(elapsed >= fillDuration) { startTime = Date.now(); localStorage.setItem('logicalWingStart', startTime); elapsed = 0; }
  ...
  function animateProgress(el, target) {
    ...
    const step=0.5;
    function stepFn() {
      if(current<target){current=Math.min(current+step,target); el.style.width=current+'%'; requestAnimationFrame(stepFn);}
    }
  ...
  function updateRewardsProgress() {
    ...
    rewardsMapping.forEach(item=>{
      ...
      if(fill && locked){
        if(unlockedBranches[item.branch]){ locked.style.display='none'; animateProgress(fill, Math.min(currentPercent,item.maxPercent)); }
        else { fill.style.width='0%'; locked.style.display='block'; }
      }
    });
  }
  ...
  // --- COLLAPSIBLE CARDS ---
  function toggleInfo(id){ const card=document.getElementById(id); if(card) card.classList.toggle('open'); }
